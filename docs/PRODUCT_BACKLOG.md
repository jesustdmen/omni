...
#### PB-020e — Validação de tempo e gaps na Triagem (frente de produto da PB-020)

| Campo | Valor |
|---|---|
| Prioridade | P1 |
| Status | **IMPLEMENTADO E VALIDADO (2026-07-03) — aceite operacional do PO pendente.** Subpágina `/conversations/:id/time_validation` + `gap_kind` (migration `20260703120000`) + regras de confirmação no model + service `ConversationTimeValidation`; suíte 947/3515/0; rubocop/brakeman 0. **Mescla/consolidação (bloco A1/A2/A3) NÃO implementada** — mínimo seguro entregue (alerta de overlap, sem soma duplicada silenciosa); exige novo contrato. Especificação em `PB-020_TRIAGEM_CONVERSAS_REQUISITOS.md` (§ "CONTRATO PB-020e"). |
| Problema que resolve | Transformar blocos de trabalho da conversa em **subtotal validado** confiável dentro da Triagem, separando execução real de gaps antes de qualquer promoção para `TimeEntry`. |
| Critério de aceite | Subpágina/seção de Triagem permite revisar blocos, confirmar execuções, classificar gaps e visualizar subtotal validado por conversa/data/turno. `confirmed` = entra no subtotal se `kind=execution`; `gap` confirmado exige `gap_kind` e nunca entra no subtotal; `draft` fica pendente; `discarded` fica fora. Cliente e task são obrigatórios para validar tempo. Duração deve existir, ser >0 e aparecer em `HH:MM:SS`. Deve ficar explícito que o subtotal ainda **não criou apontamento oficial**. |
| Decisões | • Não criar `TimeEntry` nesta fatia. • Não criar `Task`. • Não editar duração livremente; validar/classificar/consolidar blocos. • Tipos iniciais de `gap_kind`: `pernoite`, `almoco`, `lanche`, `outra_tarefa_cliente`, `aguardando_cliente`, `pausa`, `indeterminado`. • `fora_vscode` não é gap por padrão, pois pode ser execução/evidência. • Overlap é risco residual: alertar se detectado e não somar duplicado silenciosamente. • Reindex/sync posterior não invalida automaticamente validações já feitas. |
| Fora de escopo | Promoção para `TimeEntry`; rascunho/apontamento oficial; precificação/fechamento/PDF; IA classificando gaps; anexos formais; consolidação entre múltiplas conversas do mesmo cliente. |
| Dependências | PB-020d (`conversation_work_blocks`), Triagem persistida com cliente/task, ADR-023 (dia operacional Brasília). |
