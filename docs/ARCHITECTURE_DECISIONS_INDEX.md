...
| ADR-011 | Agendador externo roda; Rails lê | ✅ Aceito (+addenda 2026-06-21, 2026-06-22, 2026-06-28 e 2026-06-29) | 2026-06-16 | 3/6 | Médio | 005 | **PB-015** (Rails só lê) + **PB-016** (Omni dispara via agente no host, allowlist+timeout+token; agendamento em /settings). **Addendum 2026-06-29 (F7.7):** pipeline de coleta/normalização **internalizado** em `app/pipeline` (`run_collect.py`), A/B byte-a-byte com o legado; **RepoB deixa de ser runtime** (agente roda o nativo; mount vem de `app/pipeline/output/normalized`). RepoB segue só como referência read-only |
...
| ADR-026 | Redesign do core operacional (Rails/Hotwire) | ✅ Aceito | 2026-07-09 | PB-023 | Alto | 001,002,012,024 | core diário = 8 telas (Dashboard/Tarefas/Demandas/Clientes/Projetos/Horas/Contratos/Configurações); Tarefas vira **workspace lista+detalhe** (Turbo Frames, fatia posterior); Conversas/Triagem **fora da nav principal** nesta etapa (rotas intactas; etapa 2 futura); Sync = admin via Configurações; Apuração adiada (menu inerte); **tokens visuais oficiais consolidados no ADR** (paleta petróleo `#0e6e6b`, Space Grotesk/Hanken Grotesk/IBM Plex Mono, medidas fixas); fontes/ícones **locais** (sem CDN — CSP ADR-012); 1ª fatia (PB-023a) sem migration; **pendentes:** herança contrato→horas, etapa 2 de conversas, persistência de turnos se retomada; supersede o baseline visual F2.UI |
| ADR-027 | Omni Desktop Shell (Tauri sobre o stack web local) | ✅ Aceito | 2026-07-13 | PB-024 | Alto | 001,002,006,011,012,015,021 | shell desktop em **Tauri (Rust)** embute uma **WebView em `localhost:3030`** e **reutiliza 100% do Rails/Hotwire** (sem SPA/reescrita). Duas camadas: janela + supervisor; **MVP Docker** reaproveita o contrato do `.devstack`, runtime nativo fica para Fase 2. Frontend = web; agentes = sidecars Python; Rust = cola nativa fina. Artefato **fora de `app/`**. Absorve a pendência Desktop Runtime de 25/06 na PB-024c (health/reindexação com addenda ADR-011/021). **Segurança:** loopback, capacidades Tauri mínimas, sem shell genérico e sem remover dados no stop. **Validação futura:** PB-024a confirma Tauri ou aciona plano B Electron; assinatura/SmartScreen na PB-024d |
