...
| ADR-025 | Empresa Prestadora + Contratos (frente comercial) | ✅ Aceito (decisão; sem implementação) | 2026-06-24 | PB-019 | Alto | 023 | novas `provider_companies` e `contracts` (prestadora+cliente, projeto opcional, prioridade projeto>cliente); só `hourly`+`hourly_rate` decimal; status enum fixo (draft/active/suspended/ended); apontar não exige contrato (horas sem valor); **snapshot no fechamento** (TimeEntry não grava valor/contrato); sobreposição validada em Rails (EXCLUDE/btree_gist futuro); UI Prestadora em Configurações, Contratos na sidebar; não renomear Client→Empresa |
