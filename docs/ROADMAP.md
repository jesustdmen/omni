...
- **Pendências → roadmap:** **OP-01 (sync manual pela UI) e OP-03 (histórico de sync) ENTREGUES na PB-015 (2026-06-21)** — importação operacional pela UI (lê `output/normalized/`, não dispara pipeline; ADR-011). **PB-016 ENTREGUE/CONCLUÍDA (2026-06-22):** agendador interno de importação em Configurações + execução do pipeline pelo Omni via agente no host (addendum ao ADR-011). Restam CV-03 (títulos de sessão dedicados) e CV-10 (tags). Import em massa de turnos permanece N/A (lazy via ADR-021).
