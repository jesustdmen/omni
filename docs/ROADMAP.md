...
- **Pendências → roadmap:** **OP-01 (sync manual pela UI) e OP-03 (histórico de sync) ENTREGUES na PB-015 (2026-06-21)** — importação operacional pela UI (lê `output/normalized/`, não dispara pipeline; ADR-011); restam CV-03 (títulos de sessão dedicados), CV-10 (tags) e **PB-016** (agendador interno de importação em Configurações). Import em massa de turnos permanece N/A (lazy via ADR-021).
