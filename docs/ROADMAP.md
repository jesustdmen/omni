...
- **Entregue/publicado (`main`, 2026-06-25..29; frente de produto da PB-020):** **Central/inbox read-only de Triagem** (`68e9e8c`) + **modo triagem no detalhe** (`3ae1484`); **decisão de triagem persistida mínima** (`conversation_triages`, `99bf00f` — status open/reviewed/ignored + confirmar cliente/projeto); **criar/vincular tarefa com contexto** (`0e957bf`); **atividades manuais de 2º nível** (`conversation_activity_drafts`, `e983a29`); **sugestão por IA local** Ollama/Gemma4 isolada e testada (`18b80e2`/`de58b7e`, extrai do conteúdo real); **PB-020d blocos de trabalho** (`conversation_work_blocks`, 2026-06-29). IA sugere; humano confirma. Validação técnica (checks verdes); **produção não exercida**.
- **PB-020e — validação de tempo e gaps na Triagem (2026-07-03) — IMPLEMENTADA E VALIDADA (aceite operacional pendente):** subpágina `/conversations/:id/time_validation`; `confirmed` entra no subtotal se `execution`; gap confirmado exige `gap_kind` e nunca conta; subtotal por conversa/data/turno em `HH:MM:SS`; cliente+task obrigatórios; alerta simples de overlap; sem `TimeEntry`/`Task`/promoção oficial; mescla/consolidação fica para novo contrato.
- **Ainda NÃO iniciado nesta fase:** diário (view sob demanda); retenção; handoff externo; workspaces órfãos; cards/filtros avançados/lote/exportar da Triagem; implementação da **validação de tempo/gaps**, rascunho/promocao controlada para `TimeEntry` (frente de tempo da PB-020). Agendamento de sync já entregue na PB-016b (`/settings`).
...
- **F7.1 (2026-06-19) — CONCLUÍDA:** endurecimento de `production.rb` por ENV (`force_ssl`/`assume_ssl`/`config.hosts`/mailer host; `/up` fora do redirect) + **admin seed** opt-in/idempotente (`db/seeds.rb`, `OMNI_SEED_ADMIN`+`OMNI_ADMIN_*`; sem senha padrão; no-op sem flag). Sem schema/Solid/Kamal/Dockerfile/credentials. Suíte 279/1087/0. Detalhe em [`F7_CONTRACT_DECISIONS.md`](F7_CONTRACT_DECISIONS.md). **F7.7 (2026-06-29) — CONCLUÍDA:** pipeline NATIVO de coleta/normalização internalizado em `app/pipeline` (`run_collect.py`), A/B byte-a-byte com o legado; **RepoB deixa de ser runtime** (ADR-011 addendum 2026-06-29). **Próximas:** F7.2 Solid trifecta · F7.3 worker · F7.4 Kamal · F7.5 `/normalized` prod · F7.6 runbook.
- **Readiness de produção (produção NUNCA exercida).** **Resolvido na F7.1:** `production.rb` endurecido por ENV (`force_ssl`/`assume_ssl`/`config.hosts`/mailer host); **admin seed** opt-in/idempotente; redação de PII (F5.1.5). **Bloqueadores restantes:** schemas **Solid cache/cable** ausentes (só `db/queue_schema.rb`); `cable.yml` de prod em **Redis** (decidir `solid_cable`); **Kamal/deploy ausente** (`config/deploy.yml`/`.kamal/`); **worker de jobs** (Solid Queue) não amarrado ao deploy; **`/normalized` em produção indefinido** (→ `:stale`); **coleta:** o pipeline agora é **NATIVO do Omni** (`app/pipeline`, F7.7) e **RepoB não é mais runtime** — resta a **topologia de produção** (onde o host Windows roda a coleta + entrega do volume `:ro` no deploy); **backup/restore/rollback** de prod (runbook); **SMTP** não configurado. *(Ampliação de PII p/ CPF/telefone/IP/segredos não-rotulados = follow-up.)*
...
| M6 | Triagem, diário e sync operacional | 6 | 🟡 Em progresso (parcial) — Triagem da PB-020 entregue/publicada (base read-only + decisão persistida + criar/vincular + atividades de 2º nível + IA local de sugestão + PB-020d blocos de trabalho); PB-020e validação de tempo/gaps implementada/validada (aceite pendente); diário/retenção/handoff/órfãos, mescla de blocos e promoção a TimeEntry → pendentes |
