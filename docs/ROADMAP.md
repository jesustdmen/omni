...
- **Ainda FORA (F5.6+):** **inbox de triagem com lote/atalhos (UI-05, v1)**, syntax highlight, busca, virtualização, modal vincular (Ctrl+L, UI-09), dashboard (UI-01), **abas dinâmicas JS da task (opcional)**, tags, arquivos alterados. *(Navegação por âncoras da task entregue na F5.5; lista acionável CV-04 na F5.4; criar tarefa UI-10 na F5.3; markdown na F5.2; PII na F5.1.5.)*
