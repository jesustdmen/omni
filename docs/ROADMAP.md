...
- **Contratado, ainda NÃO implementado:** **PB-020e — validação de tempo e gaps na Triagem** (2026-07-03): `confirmed` entra no subtotal se `execution`; gap confirmado exige `gap_kind` e nunca conta; subtotal por conversa/data/turno; cliente+task obrigatórios; sem `TimeEntry`/`Task`/promoção oficial.
