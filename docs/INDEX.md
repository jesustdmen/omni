...
**Estado (2026-06-25):** M1/M2 ✅ · M3 🟢 MVP de metadados · M4 🟢 MVP manual · M5 ✅ MVP interno concluído (F5.1 render + F5.1.5 PII + F5.2 markdown + F5.3 criar tarefa + F5.4 lista acionável + F5.5 navegação por âncoras; UI-01/04/09, CV-03/05/06/10, scorer, inbox = roadmap/v1) · **M6 🟡 parcial** (frente de Triagem da PB-020 iniciada) · M7 ⬜. **Trilha ativa: Produto Operacional** (PB-003 concluída; **PB-015 entregue** — sync operacional de conversas, importação via UI lê `output/normalized/` e **não** dispara o pipeline, ADR-011; **PB-004 concluída** (a/b/c), **PB-005** (`/demands`), **PB-006** (`/clients` Empresas/Contatos + busca de CNPJ na BrasilAPI **pelo navegador** — **ADR-022**, proxy no Rails revertido em 2026-06-23) e **PB-007** (`/projects` + duplicação) entregues — **as 4 listas operacionais estão completas** (lacuna operacional da PB-001 fechada); **PB-013 CONCLUÍDA** — PB-013a (busca global na topbar — `GET /search`, agrupada por categoria) + PB-013b (preservação de contexto/navegação via concern central `return_to`, anti open-redirect; breadcrumbs acessíveis); **PB-014 ENTREGUE** — código legível de tarefa `TSK-000001` (sequence do Postgres; exibido em lista/detalhe/busca/links/selects; reabre o ADR-016 via addendum); **PB-016 CONCLUÍDA** — PB-016a (Omni dispara o pipeline via **agente no host**; o Rails/worker não roda Python; coleta+importação no "Sincronizar agora") + PB-016b (agendamento interno configurável em **`/settings`**; SolidQueue `recurring.yml`; sem Tarefa do Windows); addendum ao **ADR-011**. **Concluída em dev/local;** a coleta usa `run_pipeline.py` do **RepoB** (referência read-only, ausente em produção) = **andaime de dev** → **coleta produtiva pendente** (ADR-011 addendum 2026-06-28; F7.7). **Melhoria UX (2026-06-23):** paginação amigável (« Primeira/Última », "Mostrar tudo" + aviso) em todas as listas + turnos de `/conversations/:id`. Pendente: produção (F7). **F7.1 entregue** (endurecimento de `production.rb` + admin seed), mas **deploy/produção real NUNCA foi exercido**. **Métricas correntes: ver `PROJECT_STATUS.md`** (fonte única; não duplicadas aqui).
