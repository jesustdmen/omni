...
**Estado (2026-06-20):** M1/M2 ✅ · M3 🟢 MVP de metadados · M4 🟢 MVP manual · M5 ✅ MVP interno concluído (F5.1 render + F5.1.5 PII + F5.2 markdown + F5.3 criar tarefa + F5.4 lista acionável + F5.5 navegação por âncoras; UI-01/04/09, CV-03/05/06/10, scorer, inbox = roadmap/v1) · M6/M7 ⬜. **Trilha ativa: Produto Operacional** (PB-003a/b/c entregues — **PB-003 concluída**; listas (PB-004/005/006), PB-013/PB-014 e produção pendentes). **F7.1 entregue** (endurecimento de `production.rb` + admin seed), mas **deploy/produção real NUNCA foi exercido**. **Métricas correntes: ver `PROJECT_STATUS.md`** (fonte única; não duplicadas aqui).
