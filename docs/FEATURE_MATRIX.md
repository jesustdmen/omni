...
| WD-04 | Tarefas (+counters, página /tasks/:id) | Repo A/Mockup | 2 | MVP | ✅ Entregue (F2.3 base CRUD + /tasks/:id com apontamentos/conversas + badges) + **PB-004a:** lista `/tasks` operacional (**busca título/descrição, filtros status/tipo/cliente, paginação 10/25/50/100, ações ver/editar/excluir, estados vazios**; sem N+1) + **PB-004b:** **checklist persistente** na tarefa (adicionar/marcar/editar/excluir, contador, edição in-place sem JS). 🟡 Pendente: vínculo demanda↔tarefa (PB-004c+); código legível adiado (ADR-016/PB-014) | WD-01,WD-03 | CRUD + abas + counters |
