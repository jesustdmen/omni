...
| WD-03 | Projetos | Repo A | 2 | MVP | ✅ Entregue (F2.2 — base CRUD) + **PB-007:** lista `/projects` operacional (**busca nome/descrição, filtros cliente/status, paginação, período, orçamento, ações ver/editar/duplicar/excluir**); **status fechado (4 valores) com CHECK + labels PT-BR + select**; **duplicação transacional** (não copia datas/orçamento/tarefas); sem N+1 | WD-01 | CRUD + FK |
| WD-04 | Tarefas (+counters, página /tasks/:id) | Repo A/Mockup | 2 | MVP | ✅ Entregue (F2.3 base CRUD + /tasks/:id com apontamentos/conversas + badges) + **PB-004a:** lista `/tasks` operacional (**busca título/descrição, filtros status/tipo/cliente, paginação 10/25/50/100, ações ver/editar/excluir, estados vazios**; sem N+1) + **PB-004b:** **checklist persistente** na tarefa (adicionar/marcar/editar/excluir, contador, edição in-place sem JS). + **PB-014:** **código legível** `TSK-000001` (sequence; exibido em lista/detalhe/busca/links/selects). 🟡 Pendente: tags/assignee/due_date/estimated_hours (v1) | WD-01,WD-03 | CRUD + abas + counters |
...
| WD-08 | Usuários (migração Devise) | Repo A | 1 | MVP | ✅ Entregue (M1) — base Devise existe (model User + Devise, bcrypt 10 + re-hash). 🟦 **PB-017 — Auth/Admin seguro (single-user): IMPLEMENTADO E VALIDADO (2026-06-24), aguardando aceite do PO** — `:registerable` removido + `skip: :registrations` (sem cadastro público); reset por e-mail (token 30min + paranoid) + reset manual seguro documentado; `password_length` 10..128; rate limit login por IP + conta/e-mail; `mailer_sender` por ENV (SMTP separado); cookies SameSite=Lax/HttpOnly/Secure; seed admin opt-in preservado; addendum ao **ADR-003**; checklist secure-auth PASS/GAP/N/A. `is_active`/multiusuário/MFA/OAuth/passkeys = fora de escopo. **Não "Entregue" no gate (falta aceite manual).** | M0 ✅ | model User + Devise (custo bcrypt 10 + re-hash); migração de dados na F2 |
| WD-09 | Permissões (Pundit) | Mockup | 1 | MVP | ✅ Entregue (M1) | WD-08 | Pundit + verify_authorized; UserPolicy testada |
| WD-10 | Config/Health/Admin (tela Settings do modelo) | Repo A | — | P2 | **Runtime-switch produção/homologação + clone via HTTP/`spawn` = não migrar (ADR-015).** 🟡 (PB-001) **separar:** health/status de bases e painel admin **não** são descartados — avaliar como **OP (P1/P2)**; `/up` já existe (F7.1). 🟦 **Configurações (`/settings`) já hospeda o agendador de sync (PB-016b);** **gestão de status configurável de Tarefas/Projetos entra como PB-018** (Aprovado; depende de PB-017) — campos nome/chave/cor/ordem/ativo/finalizador; Demandas ficam fixas. | — | n/a |
...
| UI-07 | Settings de sync | Mockup | 6 | v1 | ✅ **PB-016b** (2026-06-22): agendador em **Configurações** (`/settings`) — ligar/desligar + intervalo (15min…24h); sem Tarefa do Windows. 🟦 **PB-018** (Aprovado; depende de PB-017): **gestão de status configurável** de Tarefas/Projetos em Configurações (nome/chave/cor/ordem/ativo/finalizador; status em uso não excluível livremente; Demandas fixas) | OP-03 | parcial/erro visíveis |
