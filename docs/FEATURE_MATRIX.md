...
| WD-10 | Config/Health/Admin (tela Settings do modelo) | Repo A | — | P2 | **Runtime-switch produção/homologação + clone via HTTP/`spawn` = não migrar (ADR-015).** 🟡 (PB-001) **separar:** health/status de bases e painel admin **não** são descartados — avaliar como **OP (P1/P2)**; `/up` já existe (F7.1). 🟦 **Configurações (`/settings`) hospeda o agendador de sync (PB-016b)** e a **gestão de status configurável de Tarefas/Projetos (PB-018 — ENTREGUE, `ad1a10e`)** — campos nome/chave/cor/ordem/ativo/finalizador; criar/editar/inativar/excluir-se-não-usado; Demandas ficam fixas. 🔭 **Futuro (PB-019a, ADR-025):** **Empresa Prestadora** também será gerida em Configurações. | — | n/a |

### Domínio comercial (frente Empresa Prestadora + Contratos — Etapa 0 registrada, ADR-025)

> **Etapa 0 (2026-06-24, docs-only):** contrato aprovado; **nada implementado**. Base para Cálculo → Fechamentos → Relatórios/PDF. Detalhe em `PRODUCT_BACKLOG.md §6.1` e **ADR-025**.

| ID | Feature | Origem | Fase | Prioridade | Status | Dependências | Critério de aceite |
|---|---|---|---|---|---|---|---|
| WD-11 | Empresa Prestadora (cadastro) | Novo/Produto | comercial | P1 | 🟦 **Pronto para execução (PB-019a)** — `provider_companies` (name/trade_name/cnpj único entre prestadoras/email/phone/address/active); UI em Configurações. Logo/fiscal → WD-15/PB-022. **Não** renomear Client→Empresa. | ADR-025 | CRUD + CNPJ único entre prestadoras |
| WD-12 | Contratos | Novo/Produto | comercial | P1 | 🟦 **Aprovado (PB-019b)** — `contracts` (prestadora+cliente, projeto opcional; vigência; modalidade só `hourly`; `hourly_rate` decimal obrigatório; status enum draft/active/suspended/ended; notes/active). Projeto > cliente na resolução. **Sobreposição validada em Rails** (risco residual: sem EXCLUDE no banco). UI item próprio na sidebar. | WD-11, WD-01, WD-03; ADR-025 | CRUD + validação de vigência/sobreposição |
| WD-13 | Cálculo de horas / preview | Novo/Produto | comercial | P1 | 🟦 **Aprovado (PB-020)** — resolve contrato por data (projeto>cliente); sem contrato = horas sem valor; `rounding_rule` aqui; decimais (sem float). Não grava na TimeEntry. | WD-12, ADR-023, ADR-025 | preview horas×valor por período |
| WD-14 | Fechamentos (snapshot) | Novo/Produto | comercial | P1 | 🟦 **Aprovado (PB-021)** — congela snapshot (prestadora/cliente/contrato/período/horas/valor-hora/total/regras); fonte oficial do relatório; alterar contrato depois não altera fechado. | WD-13, ADR-025 | snapshot imutável por período |
| WD-15 | Relatórios / PDF | Novo/Produto | comercial | P1/P2 | 🟦 **Aprovado (PB-022)** — consome snapshot; **logo e dados fiscais** da prestadora entram aqui. | WD-14, WD-11, ADR-025 | PDF a partir do snapshot |

> **Apontamento × contrato (decisão ADR-025):** registrar tempo (WD-07) **não depende** de contrato — sem contrato, horas existem **sem valor**; **nunca bloquear** a captura. `TimeEntry`/`Task`/`Project` **não são tocados** nas fatias de cadastro (PB-019a/b).
