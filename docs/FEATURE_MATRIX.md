...
| WD-07 | Apontamento de horas (+conversation_id) | Repo A/Mockup | 2 | MVP | ✅ Entregue (F2.5 base CRUD) + **PB-003a (`d11f099`, aceite manual):** **timer start/stop**, **cálculo automático de duração em segundos**, **paralelismo configurável** (`ALLOW_PARALLEL_RUNNING_TIMERS`, default true), **bloqueio de timer duplicado por tarefa** (índice único parcial), **Histórico de Apontamentos** operacional + ações inline (ver/editar/excluir/parar com ícones e cores). 🟡 **Pendente (PB-003b/c):** agrupamento por dia, **subtotal por dia**, retroativo assistido, timesheet/relatórios avançados | WD-04 | CRUD + soma duration |
