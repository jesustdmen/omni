...
| WD-08 | Usuários (migração Devise) | Repo A | 1 | MVP | ✅ Entregue (M1) — base Devise existe (model User + Devise, bcrypt 10 + re-hash). 🟦 **Nova frente PB-017 — Auth/Admin seguro (single-user):** endurecimento sobre a base Devise (cadastro público bloqueado, reset por e-mail + reset manual seguro, rate limit por IP/conta, checklist secure-auth PASS/GAP/N/A). `is_active`/multiusuário/MFA/OAuth/passkeys = fora de escopo. **Pronto para execução; não entregue.** | M0 ✅ | model User + Devise (custo bcrypt 10 + re-hash); migração de dados na F2 |
