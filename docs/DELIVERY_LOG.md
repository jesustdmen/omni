...
### Entregue (validado)
- **Service `WorkTimeReport`** (read-only, sem persistência): consolida **horas em segundos** (Integer; sem float) por tarefa no período, com totais geral/por-cliente/por-projeto, contagem de apontamentos e de **conversas vinculadas** (`Task#conversation_count`, evidência — sem inferir tempo de turnos, sem ler JSON). Filtros período (por `TimeEntry.date`, dia operacional Brasília — ADR-023), cliente, projeto, tarefa. Opção **"mostrar tarefas sem horas lançadas"** (com conversa vinculada → linha "Sem horas lançadas", `seconds=0`). **Timer em andamento (running) não distorce** (excluído; `duration=0`). Agregação por `group(:task_id).sum/count` + `includes(:client,:project)` (sem N+1). Scopes injetáveis (testável isolado).
- **Controller fino `WorkTimeReportsController#index`** + rota `GET /work_time_reports`; default de período = **mês atual no `Time.zone` (Brasília)**; filtros sanitizados (ids inexistentes ignorados); `skip_pundit` (read-only não-resourceful; auth Devise é a barreira — padrão `search`/`settings`).
- **UI PT-BR** (`/work_time_reports`): título "Apuração de horas", filtros (De/Até/Cliente/Projeto/checkbox sem-horas), resumo do período, tabela por tarefa (Tarefa+código, Cliente, Projeto, **Horas apuradas**, Apontamentos, **Conversas vinculadas**), `tfoot` com total, blocos "Total por cliente"/"Total por projeto". Duração via helper `duration_label` (PT-BR); datas via `local_date`. **Sem termos** billing/preview/provider/contract; **sem coluna/valor financeiro**. Sidebar: grupo **Comercial → "Apuração"**.
### Arquivos
Novos: `app/services/work_time_report.rb`, `app/controllers/work_time_reports_controller.rb`, `app/views/work_time_reports/index.html.erb`, `test/services/work_time_report_test.rb`, `test/integration/work_time_reports_test.rb`. Modificados: `config/routes.rb`, `app/components/sidebar_component.rb`, `app/controllers/application_controller.rb` (skip_pundit p/ work_time_reports), `app/assets/stylesheets/application.css`.
### Validação
`bin/rails test` **754 runs / 2872 assertions / 0 falhas / 0 erros / 0 skips** (`OMNI_RUN_PIPELINE_INTERNALLY=0`; +18: 10 service + 8 integração). rubocop **0 offenses**; brakeman **0**; `git diff --check` limpo; `zeitwerk:check` OK. **Sem migration/schema** (verificado). Smoke no dev: mês atual → 2 tarefas, 4787s, agrupado por cliente. Cobertura: soma segundos (Integer, sem float); período por `date` (fora não entra); fronteiras inclusivas; agrupamento cliente/projeto; filtros cliente/projeto; **running não distorce**; sem-horas só quando a opção liga (sem duplicar tarefa com horas); read-only (`assert_no_difference` TimeEntry/Task/Contract); auth; PT-BR; **sem valor monetário/contrato no conteúdo**; default mês atual Brasília.
### Escopo negativo (cumprido)
Sem cálculo financeiro; sem contrato (PB-020c); sem validação persistente (PB-020b); sem fechamento/snapshot (PB-021); sem PDF (PB-022); **TimeEntry/Task/Conversation não alterados**; sem gravar apuração no banco; sem migration; sem tocar sync/importação; `_origem/`/`_mockup/` intocados.
...
### Entregue (validado)
- **Tabela `provider_companies`** (uuid): `name` NOT NULL, `trade_name`, `cnpj`, `email`, `phone`, `address`, `active` (NOT NULL default true), timestamps. Índice em `name`; **índice único parcial** em `cnpj` (`WHERE cnpj IS NOT NULL`) — **único só entre prestadoras** (não cruza com `clients`).
- **Model `ProviderCompany`:** `name` presente; **CNPJ normalizado para só dígitos** (mesma regra do Client), **vazio→nil**, **único entre prestadoras** (opcional); `active` booleano; scopes `ordered`/`active`. **Não** renomeia/usa Client.
- **CRUD no padrão do Omni** (igual Clientes/Projetos — **decisão do PO**: não usar edição inline "admin"): página de **lista** (`/settings/provider-companies`, tabela com Razão social/Fantasia/CNPJ/Contato/Ativa + Ações editar/excluir) + páginas **Nova/Editar** com `_form` em card; **ativar/inativar** pelo campo Ativa no form; **excluir** sem vínculos (`rescue InvalidForeignKey` protege vínculos futuros). `ProviderCompaniesController` resourceful (index/new/create/edit/update/destroy; `policy_scope`; `return_to`/PB-013b) + `ProviderCompanyPolicy` (ADR-014) + `Scope`. Rota `resources :provider_companies, except: [:show], path: "settings/provider-companies"`. Labels PT-BR; CNPJ formatado (`format_cnpj`). **Reaproveita classes de UI existentes** (`projects-list`, `te-actions`, `form-card`, `errors`, `empty`, `back-link`) — sem CSS novo.
- **Configurações vira um HUB por domínio (decisão do PO)**: `/settings` é um índice de cards; cada domínio em sua **sub-página** — `/settings/sync` (agendador, PB-016b), `/settings/status` (status configurável, PB-018) e `/settings/provider-companies` (CRUD da prestadora). `SettingsController#index/sync/status`; `show` monolítico removido; redirects de `sync_schedules`/`configurable_statuses` e o atalho de `/sync_runs` apontam para as sub-páginas. Estrutura pronta para novos submenus sem inflar uma página única.
### Arquivos
Novos: `db/migrate/20260624130000_create_provider_companies.rb`, `app/models/provider_company.rb`, `app/controllers/provider_companies_controller.rb`, `app/policies/provider_company_policy.rb`, `app/views/provider_companies/{index,new,edit,_form}.html.erb`, `test/models/provider_company_test.rb`, `test/integration/provider_companies_test.rb`. Modificados: `config/routes.rb`, `app/controllers/settings_controller.rb`, `app/views/settings/show.html.erb`, `db/schema.rb`.
### Validação
`bin/rails test` **703 runs / 2699 assertions / 0 falhas / 0 erros / 0 skips** (`OMNI_RUN_PIPELINE_INTERNALLY=0`). rubocop **0 offenses**; brakeman **0**; `git diff --check` limpo (só avisos LF→CRLF); `zeitwerk:check` OK. Migration aplicada em dev e test. Cobertura: hub de Configurações (links para sync/status/prestadoras); lista + estado vazio; new/edit renderizam form; cria válida; name obrigatório (422 + erros); cnpj normaliza para dígitos; cnpj vazio→nil; duplicado entre prestadoras bloqueado; **cnpj igual a Client permitido**; editar; ativar/inativar; excluir sem vínculos; anônimo redirecionado (lista e criação); policy/usuário autenticado; sub-páginas sync/status funcionam (regressão PB-016b/PB-018 intacta).
### Fora de escopo (cumprido)
Sem **Contratos** (PB-019b), **Cálculo** (PB-020), **Fechamentos** (PB-021), **Relatórios/PDF** (PB-022), Desktop, Revisão de código. Sem logo/dados fiscais; sem vínculo User↔Prestadora (single-admin vê todas); sem tocar TimeEntry/Task/Project/Client; sem alterar ADR-025; PB-017/PB-018 intactas.

## 2026-06-24 — [Produto Operacional · PB-018] Status configurável (Tarefas/Projetos) + termos PT-BR — IMPLEMENTADO E VALIDADO (aguardando aceite do PO)
### Resumo
Status de **Tarefas** e **Projetos** passam a ser **configuráveis** (nome, chave, cor, ordem, ativo, finalizador), administrados em **Configurações**. A integridade fica no **banco** via **FK composta** (decisão do PO; **ADR-024**). **Demanda permanece fixa** (pending/converted) com rótulos PT-BR. **Termos PT-BR** também aplicados a **Tipo de tarefa**, **Origem** e **Prioridade** de demanda (listas fixas que apareciam em inglês). A cor do badge é renderizada de forma **CSP-safe** (`<style nonce>`, sem `style` inline). Implementação validada por checks verdes; **aceite manual do PO pendente** (gate). Não toca PB-017, Contratos, Fechamentos, Relatórios, Empresa prestadora, Conversas ou Desktop.
### Entregue (validado)
- **Tabela `configurable_statuses`** (uuid; `entity_type`∈{task,project}, `key`, `name`, `color`, `position`, `active`, `final`; índice **único (entity_type, key)**; CHECK de entity_type). **Seed** dos status atuais: Tarefas (pending→Pendente, todo→A fazer, in_progress→Em andamento, done→Concluída, canceled→Cancelada) e Projetos (planning→Planejamento, in_progress→Em andamento, completed→Concluído, on_hold→Em espera).
- **Integridade no banco (ADR-024):** `tasks.status`/`projects.status` seguem string/**key** (impacto mínimo). Discriminador constante `status_entity` ('task'/'project', CHECK + `attr_readonly`) + **FK composta** `(status_entity, status) → configurable_statuses(entity_type, key)` **ON DELETE RESTRICT**. **CHECKs fixos** de status de tasks/projects **removidos** (substituídos pela FK). Garante: só keys existentes, entidade correta e **status em uso não-excluível**.
- **Models:** `ConfigurableStatus` (validações: key **snake_case ASCII** única por entidade; nome presente; **cor por allowlist hex**; position; active; final; scopes `for_entity/ordered/active`; helpers `labels_for/colors_for/keys_for/select_options/label_for/in_use?`). Task/Project: removido enum/`STATUSES` fixos; `status_label` e allowlist de filtro vêm da tabela; validação `status_is_assignable` (novo valor exige **ativo**; valor atual inalterado aceito mesmo **inativo** → registros antigos não quebram). Demand mantém enum fixo + `STATUS_LABELS` PT-BR (Pendente/Convertida).
- **UI Configurações** (`/settings`): seções **Status de Tarefas** e **Status de Projetos** — criar, editar (nome/cor/ordem/ativo/finalizador; **key imutável**), **inativar** e **excluir só se não usado** (mensagem clara). `ConfigurableStatusesController` (create/update/destroy) + `ConfigurableStatusPolicy` (ADR-014: usuário autenticado). Rota `resources :configurable_statuses, path: "settings/statuses"`.
- **Telas consumidoras:** filtros, forms (selects só com **ativos + o atual**), badges, `show` e dashboard de Tarefas/Projetos passam a usar os **rótulos/cores configurados**; busca global idem. **Demanda** exibe Pendente/Convertida (fixo). `status_badge` ganhou `label:` para rótulo PT-BR mantendo o tom.
- **Cor do badge configurável — CSP-safe (ADR-012):** a cor escolhida **não** vai por `style` inline (a CSP restrita `style-src 'self' 'nonce-…'` bloqueia atributo `style` — era o motivo de o badge sair cinza). O badge usa uma **classe** `cfg-status--<entity>-<key>` e as cores são emitidas num bloco **`<style nonce>`** no layout (`configurable_status_styles_tag`), com o **nonce real da requisição** (mesmo mecanismo do importmap). Hex **sanitizado** no helper antes de virar CSS (Brakeman 0).
- **finalizador (`final`)**: afeta **apenas** exibição/filtros (classe visual no badge); **não** dispara regra comercial, cálculo, fechamento nem bloqueia edição (coberto por teste).
- **Termos PT-BR (listas fixas, decisão do PO nesta onda):** **Tipo de tarefa** (`Task::TYPE_LABELS`: Suporte/Dúvida/Implementação/Desenvolvimento/Comercial), **Origem** (`Demand::ORIGIN_LABELS`: Telefone/E-mail/Reunião/Chat/WhatsApp/Outro) e **Prioridade** de demanda (`Demand::PRIORITY_LABELS`: Baixa/Média/Alta) deixam de aparecer em inglês (eram `humanize`). Só exibição (mapa fixo + `*_label`); valores armazenados inalterados; sem migration. Aplicado em listas/detalhes/forms/aba Demanda. (Continuam fora de escopo: tornar esses campos *configuráveis*.)
### Arquivos
Novos: `db/migrate/20260624120000_create_configurable_statuses.rb`, `app/models/configurable_status.rb`, `app/controllers/configurable_statuses_controller.rb`, `app/policies/configurable_status_policy.rb`, `test/fixtures/configurable_statuses.yml`, `test/models/configurable_status_test.rb`, `test/integration/configurable_statuses_test.rb`, `test/integration/ptbr_terms_test.rb`, `docs/adr/ADR-024-configurable-statuses-fk-composta.md`. Modificados: `app/models/{task,project,demand}.rb`, `app/controllers/{tasks,projects,settings}_controller.rb`, `app/helpers/{application,search}_helper.rb` (badge por classe + `configurable_status_styles_tag` com nonce; `type/origin/priority` labels), `app/views/{tasks,projects,demands}/{index,show,_form}.html.erb` (conforme), `app/views/settings/show.html.erb`, `app/views/dashboard/index.html.erb`, `app/views/layouts/application.html.erb` (inclui o `<style nonce>` de status), `config/routes.rb`, `app/assets/stylesheets/application.css`, `db/schema.rb`, `docs/ARCHITECTURE_DECISIONS_INDEX.md`.
### Validação
`bin/rails test` **676 runs / 2607 assertions / 0 falhas / 0 erros / 0 skips** (com `OMNI_RUN_PIPELINE_INTERNALLY=0`; +29 testes nesta onda: 9 model + 16 integração de status + 4 de termos PT-BR). rubocop **0 offenses**; brakeman **0**; `git diff --check` limpo (só avisos LF→CRLF); `zeitwerk:check` OK. Migration aplicada no dev (seed 9 status; registros existentes válidos; FK comprovadamente **bloqueia** excluir status em uso e **permite** excluir status livre). ETAPA ZERO: dev só tinha status dentro das listas (tasks done/in_progress/pending; project in_progress) — FK criada sem violação. **CSP:** verificado que o `<style>` de cores leva o **nonce real** da requisição (== nonce do header `style-src`); badge **sem** `style` inline.
### Cobertura de teste (contrato)
Seed/fixtures criam status iniciais; key única por entidade (repetível entre entidades); key inválida rejeitada; cor não-hex rejeitada; entity_type fora da allowlist rejeitado; **não exclui status em uso** (app + FK `InvalidForeignKey`); exclui status livre; **inativo some do select de novo** mas aparece (selected) no edit do registro que o usa; labels configurados em filtro/badge de tarefas e projetos; renomear reflete na lista; **finalizador não bloqueia edição**; **Demanda fixa** (enum pending/converted, sem config, `status_label` PT-BR); auth exigida; **integridade por entidade** (task não aceita key só de project); **cor do badge** sai por classe + `<style nonce>` (sem `style` inline — CSP); **termos PT-BR** de tipo de tarefa e origem/prioridade de demanda em listas/detalhes/forms (sem inglês cru).
### Riscos residuais & premissas
- **`key` imutável** após criada (referenciada pela FK) — renomear é via `name`. Documentado (ADR-024).
- **Reatribuição em massa** de registros entre status **fora de escopo** (contrato): o usuário inativa o status ou move registros manualmente antes de excluir; a exclusão de status em uso é **bloqueada** (não há "mover N registros" automático nesta entrega).
- **Cor do badge** é emitida num `<style nonce>` por página (uma regra por status). É CSP-safe e cacheável pelo navegador; o custo é um bloco `<style>` pequeno no `<head>` (proporcional ao nº de status — hoje ~9 regras).
- **Termos PT-BR de Tipo/Origem/Prioridade** são **listas fixas** (mapas no model), **não** configuráveis. Tornar configuráveis (como o status) seria escopo novo.
- Demanda **não** virou configurável (decisão do contrato); só ganhou rótulos PT-BR (status/origem/prioridade).
- `final` é puramente visual; qualquer regra de negócio sobre "finalizado" é trabalho futuro (Fechamentos/Relatórios — **não iniciados**).
### Fora de escopo (cumprido)
Sem Demanda configurável; sem Contratos/Fechamentos/Relatórios/Empresa prestadora/Conversas/Desktop; sem renomear Client→Empresa; sem reatribuição em massa; PB-017 intacta; `_origem/`/`_mockup/` intocados.
