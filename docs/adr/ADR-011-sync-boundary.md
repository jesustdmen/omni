...
- **✅ RECONSTRUÇÃO EXECUTADA (2026-07-04, Gates 1–2C):** backup retido → reimport corrigido (+ reparo in-place publicado em `9427d4b`: **29 contaminadas → 0**) → remoção transacional das **1359 fantasmas** com rechecagem de vínculo humano na hora (**0 afetados**) → rebuild do índice (**refs 144.885 → ~40,4k; 0 de telemetria**). Resultado: **334 conversas reais**, domínio protegido intacto (15/15), LazyLoader `:ok`, **SyncSchedule religado** e sync completo controlado **sem recontaminação**. **A regra "telemetria não é conversa" permanece vigente** como contrato de importação; a lista conversacional fechada em `Sync::Sources` é o ponto único de manutenção para fontes novas.
