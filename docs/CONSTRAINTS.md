...
- **ADR-011 (addenda 2026-06-28 / 2026-06-29) — RepoB não é dependência de runtime.** `_origem/_repob` é **referência read-only** e **não existe em produção**. **F7.7 (2026-06-29):** o pipeline produtivo de coleta/normalização foi **internalizado** em **`app/pipeline/`** (entrypoint `run_collect.py`), com paridade A/B byte-a-byte; o agente roda o pipeline **nativo** e o mount `/normalized` vem de `app/pipeline/output/normalized`. **O Omni não depende mais do RepoB em runtime.** RepoB segue apenas como **referência read-only** (e fonte de A/B temporário). A saída gerada (`app/pipeline/output/`) **não é versionada** (`.gitignore`). Resta a topologia de **produção** do host (F7.2–F7.6).
