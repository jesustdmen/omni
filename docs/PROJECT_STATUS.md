...
- **Última decisão/entrega:** **F5.1 — render read-only de turnos (2026-06-18)**: UI read-only em `/conversations/:id` consumindo o `LazyLoader` via `Conversations::TurnListComponent`; `TURNS_PER_PAGE=50`; `personal`=**b1** (conteúdo oculto); só auto-escape (sem markdown/`html_safe`); CSP restrita. Validado em conversa real (177 turnos). *(commit/push pendentes de revisão.)* Antes: fatia pré-F5 (índice de turnos + loader, `ed27143`).
